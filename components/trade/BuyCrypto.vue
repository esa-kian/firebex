<template>
    <div class="main">
        <form @input="calculate">
            ...
            <div class="row no-gutters">
                <div class="col-7 offset-5 col-sm-12 offset-sm-0 col-md-7 offset-md-5 row mt-0 mb-0" id="btn">
                    <div class="percent-btn col m-0 ml-1" :class="'m'+([1,0.25].includes(n)?'l':'x')+'-1'"
                        ...
                        <input @change="limit ? putcash(decimal,n) : getAmountFromServer(decimal,n)" type="radio"
            ...
            <div class="footer">
                <a>موجودی</a>
                <a class="link" @click="limit ? putcash(decimal,1): getAmountFromServer(decimal,1)">
                    {{$toLocal(cash.toFixed($route.params.tradeTo === 'TOMAN' ? 0 : 2 ))}}
                    {{$coinUnit[$route.params.tradeTo]}}
                </a>
            </div>
            <div class="footer" v-if="limit">
                <a style="text-align: right">پایین ترین پیشنهاد فروش</a>
                <a style="text-align: left" class="link" @click="setMin">{{$toLocal(state.shopInfo.minSell)}}
                    {{$coinLabel[$route.params.tradeTo]}}</a>
            </div>
            <div class="footer">
            ...
            <button
                    ...
                    :disabled="readonly"
            >
                خرید {{$coinLabel[$route.params.tradeFrom]}}
            </button>
            <button
                    @click.prevent=""
                    class="btn"
        ...
        <shop-modal v-if="show" @close="show = false" :unitPrice="unitPrice" :totalPrice="totalPrice"
                    :amount="amount" type="BUY" :date="eDate" :limit="limit"/>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            setMin() {
                if (this.limit) {
                    this.unitPrice = this.$toLocal(this.state.shopInfo.minSell)
                    this.danger = false
                    this.calculate()
                }
            },
            localCalculate() {
                ...
                this.totalPrice = this.$toLocal((up * a).toFixed(0))
